...
# Random Forest Model for Chronological Unmixing

## Data preparation

### Load data for Valencia dated assemblages

```{r load training data}
training_data <- read_csv("data/training_data.csv", locale = locale(encoding = "ISO-8859-1"))
...
```{r load Medland data}
medland_survey2 <- read_csv("data/medland_survey2.csv")
```

### Prepare data from survey collections
```{r prepare Medland data}
medland_survey2_lithics <- medland_survey2 %>% 
  dplyr::filter(undiag.lithics < total.lithics) %>% 
  select(ID, c(13:27))

medland_survey2_info <- medland_survey2 %>% 
  dplyr::filter(undiag.lithics < total.lithics) %>% 
  mutate(assemblage = paste(study.area, "-", zone, "-", sector, "-", subsector, sep = "")) %>% 
  select(ID, study.area, assemblage)

medland_survey2_density <- medland_survey2 %>% 
  dplyr::filter(undiag.lithics < total.lithics) %>% 
  mutate(assemblage = paste(study.area, "-", zone, "-", sector, "-", subsector, sep = ""), 
...
```{r vfold10 all}
set.seed(456)
vf10.all <- vfold_cv(training_data %>% select(-ID),v=10)
```
...
#### Graph variable importance for random forest model with training set
...
```{r RF VIP all}
valencia.lithics.rf.mod %>% 
  fit(as.factor(period) ~ ., data = training_data[,2:ncol(training_data)]) %>% 
  extract_fit_engine() %>% 
...
medland.survey.rf.predicted <- 
  medland.survey.rf.fit %>% 
  predict(medland_survey2_lithics) %>% 
  bind_cols(medland_survey2_info, ., 
            predict(medland.survey.rf.fit, medland_survey2_lithics, type="prob")) %>% 
...
### Optional Graph: quantile for random forest probabilities
...
### Create files for graphing and for output

```{r Medland RF file for graphing}
# Create base file for graphing and output
medland.survey.rf.graph <-  
  left_join(select(medland_survey2, ID, study.area, zone, sector, subsector, total.lithics, area.sqm) %>% 
            mutate(assemblage = paste(study.area, "-", zone, "-", sector, "-", subsector, sep = "")), 
            medland.survey.rf.predicted) %>% 
...
```{r Medland RF graph predictions, fig.height=12, fig.width=10, warning=FALSE}
medland.survey.rf.graph %>% 
...
#### Save random forest dating file to csv
...
## ANOVA: all survey areas and each survey area
```{r}
cat("\nCanal de Navarrés survey area\n")
with(medland_survey2 %>% 
       dplyr::filter(total.lithics>0 & !is.na(visibility) & study.area == "Canal de Navarrés") %>% 
...
```{r}
totals.surveyed <- medland_survey2 %>% 
...
```{r}
pleist_early_holo <- read_csv("data/C14_S&E-Iberia_Pleistocene-Holocene.csv")

C14_SE_Iberia_all <- read_csv("data/C14_S&E-Iberia_all.csv", locale = locale(encoding = "850"))
